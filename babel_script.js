...
function init() {
    ...
    if ('Explorer' === BrowserDetect.browser && 10 >= BrowserDetect.version) {
        ...
        header.appendChild(document.createTextNode('This browser is not supported'));
    } // start dhtml for h1


    startDHTML(); // storage calls
...
var TEMPLATE_PREFIX = 'template_',
    FINAL_CHOICE = 'fin_',
    OPTION_PREFIX = 'opt_'; //Global variables
...
var parsed_obj = '',
    ...
    url = 'data/data.json',
    http = new XMLHttpRequest(); //Global onclick function to check which radio button is clicked
...
for (var i = 0, max = radios.length; i < max; i++) {
    radios[i].onclick = function () {
        ...
        stack = [];
        removeAllChoicesFromDom(-1); //start over again
...
function startDHTML() {
    var h1 = document.querySelector('.header'); // let opacity = h1.
...
function httpResponse() {
    if (http.status === 200 && http.readyState === 4) {
        ...
        bodyEle.appendChild(createCard(TEMPLATE_PREFIX + 0, '', parsed_obj.question, parsed_obj.select, ''));
...
function storeData() {
    var keyNameLS = "visit_frequency",
        keyNameC = "last_seen",
        strToDisplay1 = "This is your ",
        strToDisplay2 = " visit. ",
        strToDisplay3 = "Last seen at ";
    var lastSeenElement = document.querySelector('.visitHistory'),
        display = "",
        visitFreq = localStorage.getItem(keyNameLS),
        lastSeen = cookies.getCookie(keyNameC); //Local Stoage
    ...
    display = strToDisplay1 + localStorage.getItem(keyNameLS) + strToDisplay2; //Cookie
    ...
    cookies.setCookie(keyNameC, new Date().toString()); //Display to page
...
function removeAllChoicesFromDom(id) {
    for (var _i = currEleList.length - 1; _i > id; _i--) {
        var temp = document.getElementById(TEMPLATE_PREFIX + _i);
    ...
    var finChoiceRemove = document.getElementById(FINAL_CHOICE);
...
function removeChoiceFromDom() {
    var ele = document.getElementById(FINAL_CHOICE);
...
function addOptionActionListener(ele) {
    ele.addEventListener('click', function () {
        var clickedButtonID = this.id,
            ...
            elementID = buttonSplit[1]; //remove additonal cards
        ...
        stack.push(stack[stack.length - 1].select[buttonID]); //maintain the clicked option stack in DOM

        if (stack[stack.length - 1].question) {
            var card = createCard(TEMPLATE_PREFIX + currEleList.length, '', stack[stack.length - 1].question, stack[stack.length - 1].select, '');
            bodyEle.append(card);
        } else {
            removeChoiceFromDom();

            var _card = createCard(FINAL_CHOICE, '', stack[stack.length - 1].select[0], '', stack[stack.length - 1].url);

            bodyEle.append(_card);
            stack.pop();
        }
    });
...
function createCard(eleName, cardTitle, cardContent, cardOptions, cardLink) {
    var element = createElements('div', eleName),
        ...
        cardEle = createElements('div', '', 'class', 'card z-depth-2 blue-grey lighten-5'),
        titleEle = createElements('span', '', 'class grey-text text-darken-4', 'card-title'),
    ...
    contentEle.appendChild(content); //sets the number of options dynamically

    if (null !== cardOptions) {
        var optLen = cardOptions.length;

        if (optLen) {
            for (var _i2 = 0; _i2 < optLen; _i2++) {
                var opt = createElements('a', OPTION_PREFIX + currEleList.length + '_' + _i2, 'href', '#'),
                    optionContent = document.createTextNode(cardOptions[_i2].option);
...
function emailValidate() {
    var domEle = document.getElementById('email'),
        inputStr = domEle.value,
        reg = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;

    if (!reg.test(String(inputStr).toLowerCase())) {
        var label = document.querySelector('#yemail');
...
function phoneValidate() {
    ...
    if (!reg.test(String(inputStr).toLowerCase())) {
        var label = document.querySelector('#yphone');
        label.removeChild(label.firstChild);
